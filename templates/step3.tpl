{include file='modules_header.tpl'}
  ...
  {else}
      ...
      <input type="button" value="{$LANG.word_continue_rightarrow}" onclick="window.location='step4.php'" />

	  {else}

		  <form action="{$same_page}" method="post">
	      <div>
	        {$L.text_import_file_settings}
	      </div>
			  <p>
			    <input type="submit" name="continue" id="continue" value="{$L.phrase_import_file_upload_settings}" />
			  </p>
		  </form>
